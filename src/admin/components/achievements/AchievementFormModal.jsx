...
export default function AchievementFormModal({
  ...
  const header = mode === 'edit' ? 'Edit Achievement' : 'Add Achievement'
  const cta = mode === 'edit' ? 'Save Changes' : 'Add Achievement'

  const iconKeys = useMemo(() => listIconKeys(), [])

  const initial = useMemo(() => {
    const a = initialAchievement && typeof initialAchievement === 'object' ? initialAchievement : {}
    return {
      id: a.id || '',
      title: a.title || '',
      desc: a.desc || '',
      iconKey: a.iconKey || 'trophy',
    }
  }, [initialAchievement])
  ...
  function handleSubmit(e) {
    ...
    const payload = {
      ...
      iconKey: String(form.iconKey || 'trophy').trim() || 'trophy',
    }
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-muted">
                        Manage Achievements shown in your portfolio Achievements section.
                      </p>
                    </div>
                    <button
                      type="button"

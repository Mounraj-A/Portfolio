...
export default function AdminAchievementCard({ achievement, onEdit, onDelete }) {
  const Icon = getIconByKey(achievement?.iconKey || 'trophy', 'trophy')
  ...
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <h3 className="mt-4 font-poppins text-lg font-extrabold">{achievement?.title || 'Untitled'}</h3>
        <p className="mt-2 line-clamp-3 text-sm leading-relaxed text-muted">
          {achievement?.desc || '—'}
        </p>

        <div className="mt-4 flex justify-end gap-2 sm:hidden">
          <button type="button" onClick={() => onEdit(achievement)} className="btn-secondary">
            <Pencil className="h-4 w-4" />
            Edit
          </button>
          <button type="button" onClick={() => onDelete(achievement)} className="btn-secondary">
            <Trash2 className="h-4 w-4" />
            Delete
          </button>
        </div>

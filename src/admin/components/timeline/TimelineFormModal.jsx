...
export default function TimelineFormModal({ open, mode, initialItem, onClose, onSubmit }) {
  const header = mode === 'edit' ? 'Edit Timeline Item' : 'Add Timeline Item'
  const cta = mode === 'edit' ? 'Save Changes' : 'Add Item'

  const initial = useMemo(() => {
    const t = initialItem && typeof initialItem === 'object' ? initialItem : {}
    return {
      id: t.id || '',
      year: t.year || '',
      title: t.title || '',
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-muted">
                        Manage Timeline items shown in your portfolio Timeline section.
                      </p>
                    </div>
                    <button
                      type="button"
                    ...
                    <Field label="Title" hint="Required">
                      <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                        <input
                          ...
                          className="w-full bg-transparent text-sm text-text placeholder:text-muted/60 outline-none"
                          placeholder="e.g. Learning Spring Boot"
                          required
                        />
                      </div>
                    </Field>
                    ...
                    <Field label="Description" hint="Required">
                      <div className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                        <textarea
                          ...
                          placeholder="Write a short timeline description"
                          required
                        />
                      </div>
                    </Field>
